...
class PathsLoss(torch.nn.Module):
    def __init__(self, x, tau, dt, sinkhorn_iters, device, scale_factors, lamda_unbal=None, warm_start=False, branching_rate_fn=None):
        ...
        self.ot_losses = torch.nn.ModuleList([SinkhornLoss(torch.full((self.N, ), 1/self.N, device=self.device), self.x[i, :, :],
                                                           torch.full(
                                                               (self.N, ), 1/self.N, device=self.device), self.x[i+1, :, :],
                                                           self.tau * dt,
                                                           self.sinkhorn_iters,
...
class FitLoss(torch.nn.Module):
    ...
    def forward(self):
        self.C = (-(torch.cdist(self.x, self.y, p=2)**2) /
...
class TrajLoss(torch.nn.Module):
    def __init__(self, x0, x_obs, obs_idx, t_idx_obs, dt, tau, sigma, M, lamda_reg, lamda_cst, sigma_cst, branching_rate_fn, sinkhorn_iters, device, lamda_unbal=None, warm_start=False, scale_factors=None, scale_factors_fit=None, scale_factor_global=None):
        ...
        self.loss_fit = torch.nn.ModuleList([FitLoss(self.x[i, :, 0:obs_idx], torch.full((self.M, ), 1/self.M, device=self.device),
                                                     self.x_obs[self.t_idx_obs == i, :], torch.full(
                                                         (self.N[i], ), 1/self.N[i], device=self.device),
                                                     self.sigma,
                                                     self.scale_factors_fit[i]) for i in range(self.T)])
        self.loss_cst = ConstrainLoss(self.x[:,:,0:obs_idx], torch.full((self.M * self.T, ), 1/(self.M * self.T), device=self.device),
                                      self.x_obs[:,:], torch.full(
...
class ConstrainLoss(torch.nn.Module):
    ...
    def forward(self):
        self.C = -(torch.cdist(self.x.view(-1, self.d), self.x_obs.view(-1, self.d), p=2)
