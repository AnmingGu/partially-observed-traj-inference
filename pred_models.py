...
# dim = 2
# def Psi(x, t, device):
#     # velocity = -np.array([16.*np.ones_like(x[:, 1]), 2.*np.ones_like(x[:, 1])]).T   
#     velocity = x @ torch.tensor([[0., -1.], [0.,0.]],device=device)#-np.array([x[:, 1], np.zeros_like(x[:, 1])]).T
#     N = x.shape[0]
#     # velocity += np.random.randn(N, dim)*2*0.15
#     return velocity
...
def round_coupling(x, p, q):
    ...
    tmp1 = (torch.minimum(p / x.sum(1), torch.ones_like(p))).reshape(-1, 1) * \
        x * (torch.minimum(q / x.sum(0), torch.ones_like(q))).reshape(1, -1)
    err1 = p - tmp1.sum(1)
    err2 = q - tmp1.sum(0)
...
def entropy_est_knn(x, d, k=2):
    try:
        ...
        return float("NaN")
    eps = dist[:, -1]
    gamma = scipy.special.gamma(d/2 + 1)
    if gamma == 0:
        C = 1e9
    else:
        C = np.pi**(d/2) / gamma
    log_eps = np.log(eps).mean()
    if log_eps == -np.inf:
        log_eps = -1e9
    result = -(scipy.special.psi(x.shape[0]) -
               scipy.special.psi(k-1) + np.log(C) + d*log_eps)
    if result == np.inf:
        return 1e9
    return result
...
class SinkhornLoss(torch.nn.Module):
    ...
    def updateReg(self, eps, flush=False):
        ...
        self.K = self.getK(self.C, self.eps, self.x_w,
                           self.y_w, self.a, self.b)

    def __init__(self, x_w, x_spt, y_w, y_spt, Xi, eps, n_iter, scale_factor, lamda1=None, lamda2=None, warm_start=False, atol=1e-4, rtol=1e-4):
        ...
        self.lamda2 = lamda2
        self.iters_used = -1
        # scale factor, has same [length] units
        ...
        if self.Xi is None:
            ...
            self.updateC(self.Xi.forward(self.x_spt ), self.Xi.backward(self.y_spt ))
    ...
    def obj_dual(self, eps, lamda1, lamda2, C, x_w, y_w, u, v, a, b):
        ...
        f = -eps*u.log() + a
        g = -eps*v.log() + b
        f_cts = eps*torch.logsumexp((-C - g.view(1, -1)) /
                                    eps + y_w.log().view(1, -1), 1) - eps*self.logZ
        g_cts = eps*torch.logsumexp((-C - f.view(-1, 1)) /
                                    eps + x_w.log().view(-1, 1), 0) - eps*self.logZ
        f1 = F_indicator_star(
            f_cts, x_w) if lamda1 is None else lamda1*F_kl_star(f_cts/lamda1, x_w)
        f2 = F_indicator_star(
            g_cts, y_w) if lamda2 is None else lamda2*F_kl_star(g_cts/lamda2, y_w)
    ...
    def obj_primal(self, eps, C, K, x_w, y_w, u, v):
        ...
        gamma = torch.relu(round_coupling(
            self.get_coupling(K, x_w, y_w, u, v), x_w, y_w))
    ...
    def forward(self):
        ...
        if (not self.warm_start):
            ...
            pass
        with torch.no_grad():
            self.u, self.v, self.a, self.b, self.K = self.sinkhorn(
                self.C, self.eps, self.lamda1, self.lamda2, self.x_w, self.y_w, self.u, self.v, self.a, self.b, self.n_iter)
...
class PathsLoss(torch.nn.Module):
    def __init__(self, x, tau, dt, sinkhorn_iters, device, scale_factors, Xi, lamda_unbal=None, warm_start=False, branching_rate_fn=None):
        ...
        self.ot_losses = torch.nn.ModuleList([SinkhornLoss(torch.full((self.N, ), 1/self.N, device=self.device), self.x[i, :, :],
                                                           torch.full((self.N, ), 1/self.N, device=self.device), self.x[i+1, :, :], 
                                                               #torch.tensor(np.repeat(np.expand_dims(np.average(-Psi(self.x[i, :, :].detach().cpu().numpy(), i) * dt, axis=0), 0), self.N, axis=0), requires_grad=False,device=self.device),
                                                           Xi,
                                                           self.tau * dt,
                                                           self.sinkhorn_iters,
    ...
    def update_weights(self):
        ...
        for loss in self.ot_losses:
            loss.x_w = normalize(
                (self.branching_rate_fn(loss.x_spt)*self.dt/2).exp())
            loss.y_w = normalize(
                (-self.branching_rate_fn(loss.y_spt)*self.dt/2).exp())
...
class FitLoss(torch.nn.Module):
    ...
    def __init__(self, x, x_w, y, y_w, sigma, scale_factor):
        ...
        # user-specified scale factor, with units [length]
        self.scale_factor = scale_factor
        self.C1 = None
        self.C2 = None

    def forward(self):
        self.C1 = (-(torch.cdist(self.x, self.x, p=2)**2) /
                  (2*(self.scale_factor*self.sigma)**2)) + torch.log(self.x_w).view(-1, 1)
        self.C2 = (-(torch.cdist(self.y, self.x, p=2)**2) /
                  (2*(self.scale_factor*self.sigma)**2)) + torch.log(self.y_w).view(-1, 1)
        
        return 0.0 * torch.logsumexp(self.C1, 0).dot(self.x_w) - 1.0 * torch.logsumexp(self.C2, 0).dot(self.x_w)

    def update_sigma(self, sigma):
        self.sigma = sigma
        
class FitSinkhornLoss(torch.nn.Module):
    def __init__(self, x, y, tau, sinkhorn_iters, device, lamda_unbal=None, warm_start=False):
        super(FitSinkhornLoss, self).__init__()
        self.device = device
        self.x = x  # starting empirical marginals, dimensions (T, N, d)
        self.y = y
        self.Nx = x.shape[0]  # number of particles per timepoint
        self.d = x.shape[1]  # dimension
        self.Ny = y.shape[0]  # number of particles per timepoint
        self.tau = tau  # diffusivity
        self.lamda_unbal = lamda_unbal  # unbalanced penalty
        self.warm_start = warm_start
        self.sinkhorn_iters = sinkhorn_iters
        self.ot_loss = SinkhornLoss(torch.full((self.Nx, ), 1/self.Nx, device=self.device), self.x[ :, :],
                                                           torch.full((self.Ny, ), 1/self.Ny, device=self.device), self.y[ :, :], 
                                                               #torch.tensor(np.repeat(np.expand_dims(np.average(-Psi(self.x[i, :, :].detach().cpu().numpy(), i) * dt, axis=0), 0), self.N, axis=0), requires_grad=False,device=self.device),
                                                           None,#no prediction map needed
                                                           self.tau,
                                                           self.sinkhorn_iters,
                                                           1,
                                                           warm_start=self.warm_start,
    ...
    def update_sigma(self, tau):
        # iteratively update the tau parameter for each Sinkhorn term, following eps = tau * dt
        self.tau = tau
        #for (i, loss) in enumerate(self.ot_losses):
        self.ot_loss.updateReg(tau)
    ...
    def forward(self):
        ...
        return self.ot_loss.forward()

    def forward_primal(self):
        return self.ot_loss.obj_primal(loss.eps, loss.C, loss.K, loss.x_w, loss.y_w, loss.u, loss.v)



class TrajLoss(torch.nn.Module):
    def __init__(self, Xi, g, x0, x_obs, obs_idx, t_idx_obs, dt, tau, sigma, M, lamda_reg, lamda_cst, sigma_cst, hid_weight, branching_rate_fn, sinkhorn_iters, device, fitType = 'Sinkhorn', forwardMap = 1, backwardMap = 1, lamda_unbal=None, warm_start=False, scale_factors=None, scale_factors_fit=None, scale_factor_global=None):
        super(TrajLoss, self).__init__()
        self.device = device
        self.hid_weight = hid_weight
        self.T = x0.shape[0]  # number of timepoints
        self.d = x0.shape[2]  # dimensions
        # number of sample points (per timepoint)
        self.N = torch.unique(t_idx_obs, return_counts=True)[1]
        ...
        self.Xi = Xi
        self.scale_factors = torch.ones(
            self.T-1) if scale_factors is None else scale_factors
        self.scale_factors_fit = torch.ones(
            self.T) if scale_factors_fit is None else scale_factors_fit
        ...
        # Do Fit and Constrain losses only on the observed variables (obs_idx)
        # Reverse KL (ish)
        if self.fitType == 'KL':
            self.loss_fit = torch.nn.ModuleList([FitLoss(g(self.x[i, :]), torch.full((self.M, ), 1/self.M, device=self.device),
                                                     g(self.x_obs[self.t_idx_obs == i, :]), torch.full(
                                                         (self.N[i], ), 1/self.N[i], device=self.device),
                                                     self.sigma,
                                                     self.scale_factors_fit[i]) for i in range(self.T)])
        else:
        # Sinkhorn
            self.loss_fit = torch.nn.ModuleList([FitSinkhornLoss(g(self.x[i, :]), 
                                                     g(self.x_obs[self.t_idx_obs == i, :]), self.tau, sinkhorn_iters, device=self.device) for i in range(self.T)])
        
        stack = torch.stack([g(self.x[i, :]) for i in range(len(self.x))])
        self.loss_cst = ConstrainLoss(stack, torch.full((self.M * self.T, ), 1/(self.M * self.T), device=self.device),
                                      g(self.x_obs[:,:]), torch.full(
                                          (self.N.sum(), ), 1/(self.N.sum()), device=self.device),
                                      self.sigma_cst,
...
class ConstrainLoss(torch.nn.Module):
    def __init__(self, x, x_w, x_obs, x_obs_w, sigma, scale_factor, hid_weight, device):
    ...
    def forward(self):
        self.C1 = -(torch.cdist(self.x[:,:,:].view(-1, self.d), self.x_obs.view(-1, self.d), p=2)
                   ** 2)/(2*(self.scale_factor*self.sigma)**2) + torch.log(self.x_obs_w).view(1, -1)
        out = -torch.logsumexp(self.C1, 1).dot(self.x_w)
#         M = self.x.shape[1]
#         xww = torch.full((M , ), 1/(M), device=self.device)
#         for i in range(self.x.shape[0]):
#         # Add term controlling variance of hidden unit
#             C2 = -(torch.cdist(self.x[i,:,1:2].view(-1, self.d), self.x[i,:,1:2].view(-1, self.d), p=2)
#                    ** 2)/(2*(self.scale_factor*self.sigma)**2) + torch.log(xww).view(1, -1)
            
#             out +=   -self.hid_weight * torch.logsumexp(C2, 1).dot(xww) / self.x.shape[0] #logsumexp
        return out
...
def optimize(model, n_iter, tau_final, eta_final, sigma_final, temp_init, temp_ratio, dim, print_interval=50, inspect_dynamics=False, **kwargs):
    obj = []
    obj_primal = []
    temp_curr = temp_init
    ...
    optim = LangevinGD(model.parameters(), eta=eta_t,
                       sigma2=2*tau_t*model.lamda_reg, **kwargs)
    ...
    for i in range(n_iter):
        ...
        with torch.no_grad():
            ...
            x = model.x.cpu().numpy()
            loss_primal = model.forward_primal() + tau_t*model.lamda_reg * \
                sum([entropy_est_knn(x[i, :, :], d=dim, k=2)
        ...
        if i % print_interval == 0:
            avg_iters = np.array(
                [l.iters_used for l in model.loss_reg.ot_losses]).mean()
            
            if inspect_dynamics:
                x_vel = (model.x.reshape(-1, dim)[:, 2].cpu()).detach().numpy().mean()
                y_vel = (model.x.reshape(-1, dim)[:, 3].cpu()).detach().numpy().mean()
                print("Iteration %d, Loss = %0.3f, Primal loss = %0.3f, Avg. iters = %0.3f, eta = %0.3f, temp = %0.3f, Pred x vel avg. = %0.3f, Pred y vel avg. = %0.3f" % (
                i, loss, loss_primal, avg_iters, eta_t, max(1, temp_curr), x_vel, y_vel))
            else:
                print("Iteration %d, Loss = %0.3f, Primal loss = %0.3f, Avg. iters = %0.3f, eta = %0.3f, temp = %0.3f" % (
                i, loss, loss_primal, avg_iters, eta_t, max(1, temp_curr)))
