...
from utils import sde_integrate
...
class Simulation(TimeSeries):
    ...
    :param Xi: known divergence-free drift :math:`(x, t) \\mapsto \\Xi(x, t)`
    ...
    """
    def __init__(self, V, dV, Xi, N, T, d, D, t_final, ic_func, pool, birth_death = False, birth = None, death = None):
        self.V = V
        self.dV = dV
        self.Xi = Xi
    ...
    def sample(self, steps_scale = 1, trunc = None):
        ...
        def F(i):
            snap, snap_mask = sde_integrate(dV = self.dV, Xi = self.Xi, nu = self.D, x0 = ic_all[i],
    ...
    def sample_trajectory(self, steps_scale = 1, N = 1):
        ...
        snap, snap_mask = sde_integrate(dV = self.dV, Xi = self.Xi, nu = self.D, x0 = ic,
            b = self.birth, d = self.death, 
            t = self.t_final, 
    ...
    def __copy__(self):
        return Simulation(V = self.V, dV = self.dV, Xi = self.Xi, N = self.N, T = self.T, d = self.d, D = self.D, t_final = self.t_final,
