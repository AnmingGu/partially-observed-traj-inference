...
def sde_integrate(dV, Xi, nu, x0, t, steps, birth_death = False, b = None, d = None, g_max = 50, snaps = None):
    """Integrate SDE using Euler-Maruyama method (with birth-death)
    
    :param dV: function `dV(x, t)` specifying the drift field
    :param Xi: function `Xi(x, t)` specifying the divergence-free drift field
    ...
    for i in range(0, steps):
        ...
        Xi_vec[x_mask, :] = Xi(x[x_mask, :], t_current)
        x[x_mask, :] = x[x_mask, :] - dV_vec[x_mask, :]*dt - Xi_vec[x_mask, :]*dt + np.sqrt(nu)*dW(dt, x[x_mask, :].shape)
